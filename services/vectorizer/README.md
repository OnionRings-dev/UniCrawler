...
`parser:out` messages are `vectorize.request.v1` envelopes containing only `document_id` and `version_id`; the vectorizer uses both values to read the immutable Markdown version stored by the parser. Qdrant point IDs are deterministic, so processing the same message again updates the same chunks instead of duplicating them.
