...
class PostgresStore(AbstractContextManager["PostgresStore"]):
    ...
    def pipeline_event(
        ...
        with self._pool.connection() as conn:
            conn.execute(
                """
                INSERT INTO pipeline_events (service, event_type, document_id, version_id, payload)
                VALUES (%s, %s, %s, %s, %s)
                """,
                (service, event_type, document_id, version_id, Json(payload or {})),
            )
...
def fetch_document_version(
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            ...
            LIMIT 1
            """,
            (message.document_id, message.version_id),
        )
        row = cur.fetchone()
