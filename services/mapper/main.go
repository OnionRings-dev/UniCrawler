...
func main() {
	...
	startHealthServer(cfg.HTTPAddr, "mapper", logger)
	if err := db.heartbeat(ctx, "mapper", "ready", map[string]any{"input_queue": cfg.InputQueue, "output_queue": cfg.OutputQueue}); err != nil {
	...
	for ctx.Err() == nil {
		...
		request, err := contracts.ParseCrawlRequest(item[1])
		if err != nil {
			logger.Warn("discarding invalid crawl request", "value", item[1], "err", err)
			continue
		}

		seed, err := normalizeURL(request.SeedURL, nil)
		if err != nil {
		...
		if _, err := db.upsertURLs(ctx, domain.ID, run.ID, []string{seed.String()}); err != nil {
			logger.Error("seed url upsert failed", "domain", job.DomainKey, "err", err)
			continue
		}
		job.DomainID = domain.ID
		job.RunID = run.ID
		_ = db.pipelineEvent(ctx, "mapper", "crawl_started", domain.ID, run.ID, 0, map[string]any{"seed_url": seed.String()})
		...
		_ = db.pipelineEvent(finishCtx, "mapper", "crawl_finished", domain.ID, run.ID, 0, map[string]any{"pages": stats.Pages, "links": stats.Links, "errors": stats.Errors})
		finishCancel()
...
func replay(ctx context.Context, cfg config, logger *slog.Logger, rdb *redis.Client, db *store, args []string) error {
	...
	total, err := db.replayDomain(ctx, domain, cfg.BatchSize, func(batch []urlRecord) error {
		items := make([]interface{}, len(batch))
		for i, record := range batch {
			payload, err := contracts.MarshalEnvelope(contracts.ParseRequestType, contracts.ParseRequest{
				URLID:      record.ID,
				DomainID:   record.DomainID,
				CrawlRunID: record.LastCrawlRunID,
			})
			if err != nil {
				return err
			}
...
func crawl(ctx context.Context, cfg config, logger *slog.Logger, rdb *redis.Client, db *store, client *http.Client, job crawlJob) crawlStats {
	...
	discovered := make(chan urlRecord, cfg.BatchSize*2)
	var workers sync.WaitGroup
	var stats crawlStats
	var statsMu sync.Mutex

	addURLs := func(urls []*url.URL) []urlRecord {
		if len(urls) == 0 {
			return nil
		}
		...
		if len(added) > 0 {
			records, err := db.upsertURLs(ctx, job.DomainID, job.RunID, added)
			if err != nil {
				if ctx.Err() == nil {
					logger.Error("postgres url upsert failed", "run_id", job.RunID, "items", len(added), "err", err)
				}
				return nil
			}
			return records
	...
	sendDiscovered := func(records []urlRecord) bool {
	...
	go func() {
		defer writer.Done()
		flushParseRequests(ctx, rdb, cfg.OutputQueue, cfg.BatchSize, discovered, logger)
	}()
...
func flushParseRequests(ctx context.Context, rdb *redis.Client, queue string, batchSize int, records <-chan urlRecord, logger *slog.Logger) {
	...
	for record := range records {
		...
		if err != nil {
			logger.Error("parse request marshal failed", "url_id", record.ID, "err", err)
			continue
		}
...
func startHealthServer(addr string, service string, logger *slog.Logger) {
	...
	mux.HandleFunc("/metrics", func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = fmt.Fprintf(w, "unicrawler_node_uptime_seconds{service=%q} %.0f\n", service, time.Since(started).Seconds())
	})
	go func() {
