...
def dashboard_payload() -> dict[str, Any]:
    ...
    counts = {
        ...
        "pipeline_events": table_count("pipeline_events"),
        "node_metrics": table_count("node_metrics"),
    }
    recent_minutes = 10
...
def enqueue_urls(raw_values: list[str]) -> dict[str, Any]:
    ...
    messages = [
        json.dumps(
            {"type": "crawl.request.v1", "version": 1, "payload": {"seed_url": value}},
            ensure_ascii=False,
        )
        for value in values
    ]
    length = redis_client().lpush(queue, *messages)
...
def replay_urls(domain_id: int, queue: str, limit: int | None) -> int:
    ...
    pushed = 0
    query = """
        SELECT id, domain_id, COALESCE(last_crawl_run_id, 0) AS crawl_run_id
        FROM urls
        WHERE domain_id = %s
        ORDER BY id
    """
...
def node_statuses() -> list[dict[str, Any]]:
    if not table_exists("node_heartbeats"):
        return []
    with db_conn().cursor() as cur:
        cur.execute(
            """
            SELECT service, status, details, last_seen_at
...
def qdrant_summary() -> dict[str, Any]:
    url = cfg().qdrant_url.rstrip("/") + "/collections"
    try:
    ...
    return {"ok": True, "collections": len(collections)}
