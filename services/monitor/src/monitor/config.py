...
def load_config() -> Config:
    return Config(
        ...
        mapper_input_queue=env_string("MAPPER_INPUT_QUEUE", env_string("MAPPER_INPUT_QUEUE_NAME", "mapper:in")),
        mapper_output_queue=env_string("MAPPER_OUTPUT_QUEUE", env_string("MAPPER_OUTPUT_QUEUE_NAME", "mapper:out")),
        parser_output_queue=env_string("PARSER_OUTPUT_QUEUE", env_string("PARSER_OUTPUT_QUEUE_NAME", "parser:out")),
        vectorizer_processing_queue=env_string("VECTORIZER_PROCESSING_QUEUE", "vectorizer:processing"),
        vectorizer_failed_queue=env_string("VECTORIZER_FAILED_QUEUE", "vectorizer:failed"),
        vectorizer_oversized_queue=env_string("VECTORIZER_OVERSIZED_QUEUE", "vectorizer:oversized"),
        replay_batch_size=env_int("MONITOR_REPLAY_BATCH_SIZE", env_int("REPLAY_BATCH_SIZE", 1000)),
        qdrant_url=env_string("QDRANT_URL", "http://qdrant:6333"),
        admin_username=env_string("MONITOR_ADMIN_USERNAME", "admin"),
        admin_password_hash=env_string("MONITOR_ADMIN_PASSWORD_HASH", ""),
        session_secret=env_string("MONITOR_SESSION_SECRET", "change-me"),
        secure_cookies=env_string("MONITOR_SECURE_COOKIES", "false").lower() == "true",
    )
